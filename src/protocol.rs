...
impl<'a> Socks4IO<'a> {
    ...
    fn read_socks_info(&mut self) -> Socks4Info {
        ...
        let tup = s
            .unpack_from(&mut self.stream)
            .expect("can't decode socks4 header");
        return Socks4Info {
            version: tup.0,
            command: tup.1,
            port: tup.2,
    ...
    fn write_socks_response(&mut self, response: u8) {
        let s = structure!("BBH4s");
        let buf: Vec<u8> = s
            .pack(0, response, 0, &vec![0; 4])
            .expect("can't encode socks4 header");
        self.stream
            .write_all(&buf)
            .expect("not all message written to socket");
    }
